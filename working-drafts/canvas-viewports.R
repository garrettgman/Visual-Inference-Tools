...
loadViewports <- function(canvas, x, y = NULL) {
	...
	canvas$viewports <- makeSimpleViewports(x.scale, n.y)
}
...
makeSimpleViewports <- function(x.scale, n.y) {
	frameVP <- plotViewport(c(1,1,1,1), name = "canvas.frame")
	graphs <- graphsBoxVP(x.scale = x.scale, n.y = n.y) 
	vpTree(frameVP, vpList(graphs))
}
...
graphsBoxVP <- function(x.scale, n.y = 1, name = list("graphs"), 
	...
	data.vp <- dataDisplay(x.scale, n.y, layout.pos.row = 1, name = c(name, "data"))
	sample <- dataDisplay(x.scale, n.y, layout.pos.row = 2, name = c(name, "sample"))
	statistic <- dataDisplay(x.scale, layout.pos.row = 3, name = c(name, "stat"))
	
	vpTree(graphsBox, vpList(data.vp, sample, statistic))
}

dataVP <- function(x.scale = x.scale, n.y = 1, name = NULL, 
	layout.pos.row = NULL, layout.pos.col = NULL){
	...
	for (i in 1:n.y) {
		data.vps[[i]] <- dataViewport(xscale = x.scale, yscale = c(0,1), 
			layout.pos.row = n.y - i + 1, name = paste(name, i, sep = "."))
	}	
		
	vpTree(frame, vpList(stat, vpTree(plot, do.call("vpList", data.vps))))
}
